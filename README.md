...
Вероятно, эту систему нет смысла использовать при малом числе индексируемых файлов (менее 5...10 тысяч). Т.к., несмотря на очень быстрый поиск ключевых слов, ее индексы занимают достаточно много места на жестком диске (около 400 МБ). Для сайтов с малым числом файлов целесообразно использовать более ранние версии (2.5 и т.д.). 
