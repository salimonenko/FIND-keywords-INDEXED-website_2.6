...
// 7.12. Создаем индекс-каталоги, в них создаем индекс-файлы, а в последние записываем индекс-номера путей к файлам сайта (если таких номеров еще не было записано)
        if(($number = array_search($ALL_files_Arr[$i], $ALL_files_Arr)) !== false){ // Определяем число-индекс, присутствующий у имени файла (из содержимого которого выше были получены слова-метафоны). Он находится после |  (в файле files.txt)
...
// Массив ключей, соответствующих текущему метафону $body_Arr[$k] (их там м.б. несколько; т.е. учитываем ОДИНАКОВЫЕ метафоны в целях последующего ранжирования)
                    ...
                    $rez = LAST_met_2_index($body_Arr[$k], count($body_Arr_k_Arr), '', $file_name, $path_DIR_name_TMP, $number, $ALL_files_Arr, $JS_manage_mes);
