...
// 2.6. Создаем логическое выражение для последующей оценки eval()
        $a = array_values($indexes_Arr_num_Arr_j);

        $bool_string = ''; // При НЕчетком поиске будет что-то типа $a[0] || $a[1] || $a[2] || $a[3] || $a[4] || ...
        for($i=0; $i < sizeof($a); $i++){
            if($i > 0){ // Для НЕчеткого поиска
                $bool_string .= ' || ';
            }
            $bool_string .= '$a['. $i. ']';
        }
...
// 2.7. Теперь следует объединить ранги при помощи функции array_mer__ge()) для одинаковых индексов и создать единый массив для них (это для каждого слова, с учетом его разных словоформ в случае НЕчеткого поиска)
        $tmp_Arr = array();
